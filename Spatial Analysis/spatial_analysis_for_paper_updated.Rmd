...
per_adj_p_data = tryCatch(expr = {pi0est(unlist(ends_p_data))}, error = function(e){not_able()})
trueProp =tryCatch(expr = {1-per_adj_p_data$pi0}, error = function(e){not_able()})
...
#Another way of accounting for multiple testing
#Should give out percentage of null p-values (we are interested in 1-nullper)
per_adj_anova = tryCatch(expr = {pi0est(unlist(anova_ends_data))}, error = function(e) {not_able()})
truePropA = tryCatch(expr = {1-per_adj_anova$pi0}, error = function(e) {not_able()})
...
```{r}
#write.table(truePVal, file = "prop_true_pval_updated_optimal.tsv", sep = "\t", row.names=FALSE) 
```
