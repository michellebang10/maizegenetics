---
title: "Control Spatial Analysis for Paper"
...
filenames = list.files("/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/all_ear_xml", pattern="*.xml", full.names=TRUE)
```
...
```{r}
#This takes the longest time to run depending on the number of files.
...
per_adj_p_data = pi0est(unlist(ends_p_data), lambda = .3) 
...
```{r}
write.table(ordered_df, file = "control_spatial_analysis_for_paper.tsv", sep = "\t", row.names=FALSE)
```

```{r}
write.table(truePVal, file = "control_true_prop_optimized.tsv", sep = "\t", row.names=FALSE)
```
