---
title: "R84A12 xml to glm cleaned"
...
filenames = list.files("/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/male", pattern="*.xml", full.names=TRUE)
```

If all files are in one folder, you do not need to do what is below. In my case, some of the files are in different folders, so I am appending the specific files to my list. 
```{r}
filenames <- append(filenames, "/Users/michellebang/st research lab/spatialanalysis/LabWorkNathan-master/Data_for_Analysis/all_xml_files/X401x492-2m1.xml")

filenames <- append(filenames, "/Users/michellebang/st research lab/spatialanalysis/LabWorkNathan-master/Data_for_Analysis/all_xml_files/X401x492-3m1.xml")

filenames <- append(filenames, "/Users/michellebang/st research lab/spatialanalysis/LabWorkNathan-master/Data_for_Analysis/all_xml_files/X403x492-1m4.xml")
```
...
write.table(fullDf1, file = "R84A12_glm.tsv", sep = "\t", row.names=FALSE) 
```
...
write.table(endsfullDf1, file = "R84A12_glm_ends.tsv", sep = "\t", row.names=FALSE) 
```
...
write.table(sqfullDf1, file = "R84A12_glm_sq.tsv", sep = "\t", row.names=FALSE) 
```
...
write.table(sqfullendsDf1, file = "R84A12_glm_sq_ends.tsv", sep = "\t", row.names=FALSE) 
```
...
```
```{r}
write.table(anova_df, file = "R84A12_anova.tsv", sep = "\t", row.names=FALSE) 
```
...
```{r}
write.table(anova_ends_df, file = "R84A12_anova_ends.tsv", sep = "\t", row.names=FALSE) 
```

\newpage
--

Part 6: Testing female data. 

6.1: Without ends removed. 
```{r}
#load file names based on your folder
ffilenames = list.files("/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/female", pattern="*.xml", full.names=TRUE)
```

```{r}
ffilenames <- append(ffilenames, "/Users/michellebang/st research lab/spatialanalysis/LabWorkNathan-master/Data_for_Analysis/all_xml_files/X492-1x402.xml")

ffilenames <- append(ffilenames, "/Users/michellebang/st research lab/spatialanalysis/LabWorkNathan-master/Data_for_Analysis/all_xml_files/X492-2x402.xml")

ffilenames <- append(ffilenames, "/Users/michellebang/st research lab/spatialanalysis/LabWorkNathan-master/Data_for_Analysis/all_xml_files/X492-3x402.xml")
```

```{r}
#Creating a list of file names to keep track of. 
ftitleList <- as.list(basename(ffilenames))
```

```{r}
fcoords = lapply(ffilenames, xml_to_coordinates)

#removes faulty xml from the files. 

sequence = rev(seq(1:length(fcoords)))

#checks if there will be an error when coordinates_to_xbins runs. If there is, it removes it from filenames 
for (i in sequence) {
  if (nrow(fcoords[[i]]) == 0)  {
    fcheck = ffilenames[i] #no files are removed here.
    ffilenames = ffilenames[-i]
    ftitleList = ftitleList[-i]
  }
}
```

```{r}
#print(fcheck) #nothing removed
```
...
```{r}
#the 0 observation issue is caused by files that don't use 1 and 2 as their marker numbers which is a requirement for xml_to_coordinates
fbin_data = lapply(fcoords, function(x) {coordinates_to_xbins(x,16)} )

sequence = rev(seq(1:length(fbin_data)))

#removes the files that break the glm, namely the ears with only one kind of kernel allele. At most removes the cases with very few WT or GFP which are by definition odd ears. 

for (i in sequence) {
  sumWT = sum(fbin_data[[i]]$WT)
  sumGFP = sum(fbin_data[[i]]$GFP)
    if ((sumWT < 5) | (sumGFP < 5)) {
      fbin_data = fbin_data[-i]
      fcheck = ffilenames[i]
      ffilenames = ffilenames[-i]
      ftitleList = ftitleList[-i]
    }
...
#turn the data into a glm
fglm_data = lapply(fbin_data, function(x) {glm(cbind(GFP,WT) ~ bins,family = quasibinomial(link = "logit"), data = x)})


#extract the p values
fpv_data = lapply(fglm_data, function(x) {(summary(x)$coefficients)})

fp_data = lapply(fpv_data, function(x) {x[8]})

#comment this out to look at the non adjusted values, you can swap the method by changing 'method = fdr'
#p_data = p.adjust(p_data,method = "fdr")

fp_df <- data.frame(matrix(unlist(fp_data), nrow=length(fp_data), byrow=T))
names(fp_df)[names(fp_df) == colnames(fp_df[1])] = "p_value"

```

```{r}
ftitleDf <- as.data.frame(t(ftitleList))
ftitleDf <- t(ftitleDf)
colnames(ftitleDf)[1] = "Name"
ffullDf = cbind(ftitleDf, fp_df)
```
...
Trying to get total count of GFP and WT across the whole ear. 
```{r}
#the 0 observation issue is caused by files that don't use 1 and 2 as their marker numbers which is a requirement for xml_to_coordinates
f_full_data = lapply(fcoords, function(x) {coordinates_to_xbins(x,1)} )

sequence = rev(seq(1:length(f_full_data)))

#removes the files that break the glm, namely the ears with only one kind of kernel allele. At most removes the cases with very few WT or GFP which are by definition odd ears. 

for (i in sequence) {
  sumWT = sum(f_full_data[[i]]$WT)
  sumGFP = sum(f_full_data[[i]]$GFP)
    if ((sumWT < 5) | (sumGFP < 5)) {
      f_full_data = f_full_data[-i]
      fcheck = ffilenames[i]
    }
}
```

Getting the transmission rate and binding to data frame. 
```{r}
fTransmissionRate <- c()
for (i in 1:15) {
  fTransmissionRate <- append(fTransmissionRate, f_full_data[[i]]$GFP/(f_full_data[[i]]$GFP + f_full_data[[i]]$WT))
}

ffullDf <- cbind(ffullDf, fTransmissionRate)
```

Taking the negative log of the p-value from GLM. 
```{r}
ffullDf <- ffullDf %>%
  mutate(neglogP = -log(p_value))
```

Plotting the transmission rate and negative log of p-value together. 
```{r}
ggplot(data = ffullDf, aes(x = fTransmissionRate, y = neglogP)) +
  geom_point()
```

```{r}
ggplot(data = ffullDf, aes(p_value)) +
      geom_histogram(binwidth = .05) +
      xlab("P-Values") +
      ylab("Count") +
      ggtitle("P-Values of all ear families without adjustment (per ear)")
...
Run this code to get .tsv file of results. 
```{r}
ffullDf1 = as.matrix(ffullDf)
write.table(ffullDf1, file = "fem_R84A12_glm.tsv", sep = "\t", row.names=FALSE) 
```

6.2: With ends removed. 

We are taking the same bin data used for our first analysis.
```{r}
#start chopping off the ends
sequence = seq(1,length(fbin_data))
#sequence2 = seq(2,15)
#sequence3 = seq(1,14)
...
```{r}
for (i in sequence) {
    fbin_ed[[i]] = data.frame("bins" = fbin_data[[i]]$bins[2:15], "WT" = fbin_data[[i]]$WT[2:15] , "GFP" = fbin_data[[i]]$GFP[2:15])
}
```
...
```{r}
#turn the data into a glm
fglm_data_ends = lapply(fbin_ed, function(x) {glm(cbind(GFP,WT) ~ bins,family = quasibinomial(link = "logit"), data = x)})


#extract the p values
fpv_data_ends = lapply(fglm_data_ends, function(x) {(summary(x)$coefficients)})

fp_data_ends = lapply(fpv_data_ends, function(x) {x[8]})

#comment this out to look at the non adjusted values, you can swap the method by changing 'method = fdr'
#p_data = p.adjust(p_data,method = "fdr")

fp_df_ends <- data.frame(matrix(unlist(fp_data_ends), nrow=length(fp_data_ends), byrow=T))
names(fp_df_ends)[names(fp_df_ends) == colnames(fp_df_ends[1])] = "p_value"

```

```{r}
ffullendsDf = cbind(ftitleDf, fp_df_ends)
```
...
Getting the transmission rate and binding to data frame. 
```{r}
ffullendsDf <- cbind(ffullendsDf, fTransmissionRate)
```

Taking the negative log of the p-value from GLM. 
```{r}
ffullendsDf <- ffullendsDf %>%
  mutate(neglogP = -log(p_value))
```

Plotting the transmission rate and negative log of p-value together. 
```{r}
ggplot(data = ffullendsDf, aes(x = fTransmissionRate, y = neglogP)) +
  geom_point()
```

```{r}
ggplot(data = ffullendsDf, aes(p_value)) +
      geom_histogram(binwidth = .05) +
      xlab("P-Values with ends chopped off") +
      ylab("Count") +
      ggtitle("P-Values of all ear families without adjustment (per ear)")
...
Run this code to get .tsv file of results. 
```{r}
ffullendsDf1 = as.matrix(ffullendsDf)
write.table(ffullendsDf1, file = "fem_R84A12_glm_ends.tsv", sep = "\t", row.names=FALSE) 
```

\newpage
---

Part 7: Looking at more graphs.
...
```{r}
pdf(file = "/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/R84A12_xcoordsbytr.pdf",
    width = 4,
    height = 4,)
for (i in 1:length(bin_data)) {
...
```{r}
pdf(file = "/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/R84A12_xcoordsbytr_linear.pdf",
    width = 4,
    height = 4,)
for (i in 1:length(bin_data)) {
  print(ggplot(data = bin_w_trate[[i]], aes(x=bins, y = rate)) +
...
```
```{r}
pdf(file = "/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/R84A12_xcoordsbytr_quad.pdf",
    width = 4,
    height = 4,)
for (i in 1:length(bin_data)) {
  print(ggplot(data = bin_w_trate[[i]], aes(x=bins, y = rate)) +
...
```{r}
pdf(file = "/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/R84A12_xcoordsbytr_ends.pdf",
    width = 4,
    height = 4,)
for (i in 1:length(bin_ed)) {
...
```{r}
pdf(file = "/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/R84A12_xcoordsbytr_linear_ends.pdf",
    width = 4,
    height = 4,)
for (i in 1:length(bin_ed)) {
  print(ggplot(data = bin_w_trate_ed[[i]], aes(x=bins, y = rate)) +
...
```
```{r}
pdf(file = "/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/R84A12_xcoordsbytr_quad_ends.pdf",
    width = 4,
    height = 4,)
for (i in 1:length(bin_ed)) {
  print(ggplot(data = bin_w_trate_ed[[i]], aes(x=bins, y = rate)) +
